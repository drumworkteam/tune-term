...
function compile(i, j, size) {
  ...
  const result = convertBigIntToSmallIntegerArray(size, x)
  const chunks = result.map((x, i) => i % 2 === 0 ? iT[x] : oT[x]).join('')
  const chunked = chunkArray(chunks.split(''), 4).map(x => x.join('')).join(':')
  return chunked
}

function convertBigIntToSmallIntegerArray(size, n, mod = 16n) {
  if (!n) return [0]
  let x = size / 2
